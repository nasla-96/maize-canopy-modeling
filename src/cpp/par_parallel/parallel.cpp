...
// Function to read config file: geometry path + location info
void readConfigFile(
    ...
    // Read location info (with defaults if not in the file)
    latitude  = pt.get<float>("Location.latitude", 42.03f);
    longitude = pt.get<float>("Location.longitude", -93.62f);
    UTC       = pt.get<int>("Location.utc_offset", 5);
}
...
int main(int argc, char* argv[]) {
    ...
    // Read OBJ file path and location info from config file
    ...
    readConfigFile(configFilePath, fieldPath, latitude, longitude, UTC);
    ...
    // ---------- Per-simulation log files (parallel-safe) ----------
    fs::path objPath(fieldPath);
    
    // Create a logs folder next to the OBJ file (e.g., canopies/helios_logs/)
    fs::path outDir = objPath.parent_path() / "helios_logs";
    fs::create_directories(outDir);
    
    // Unique filenames per simulation (based on OBJ stem)
    fs::path hourlyCsv = outDir / (objPath.stem().string() + "_hourly.csv");
    fs::path totalCsv  = outDir / (objPath.stem().string() + "_total.csv");
    
    // Open streams (append mode so reruns add lines)
    std::ofstream csvFile(hourlyCsv.string(), std::ios::app);
    std::ofstream csvFile1(totalCsv.string(), std::ios::app);

    if (!csvFile.is_open() || !csvFile1.is_open()) {
        std::cerr << "ERROR: Could not open log files in: " << outDir << std::endl;
        std::cerr << "hourlyCsv: " << hourlyCsv << std::endl;
        std::cerr << "totalCsv: "  << totalCsv  << std::endl;
        return 1;
    }
    
    // Optional: write header once per file (only if file was empty at open)
    if (csvFile.tellp() == 0) {
        csvFile << "chromosome_name,hour,PAR_plant,PAR_ground\n";
    }
    if (csvFile1.tellp() == 0) {
        csvFile1 << "chromosome_name,PAR_total\n";
    }
    // --------------------------------------------------------------
    ...
    for (int hour = 9; hour < 19; hour++) {
        ...
        // if ((hour % 3) == 0) csvFile.flush();   // every 3 hours
    }
